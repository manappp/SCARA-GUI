...
double Kp = 100, Ki = 0.00001, Kd = 0.0;
...
void setup() {
  ...
  myPID.SetOutputLimits(-9000, 9000);
...
void parseData(String data) {
  ...
  if (comma == -1) {
    Setpoint = data.toFloat();     // input manual
    Serial.print("Manual SP = ");
    Serial.println(Setpoint);
    return;
  }

  float sp1 = data.substring(0, comma).toFloat();  // untuk slave 1
  Setpoint = sp1;

  Serial.print("Master SP1 = ");
  Serial.println(Setpoint);
}
